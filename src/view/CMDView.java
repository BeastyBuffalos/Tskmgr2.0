	...
	protected void edit( String edit, CMDView thetest) {
		...
		for(int i = 0; i < 25; //the tasklist.size
			...
			if( wtask.contains("w") //tasklist.getname
				...
				System.out.println("What would you like to change about it? Name, Date, Time Expected, or manual placement in the list?");
				...
				} else if( ctask.contains("placement") || ctask.contains("Placement")) {
					System.out.println("Numerically, Where would you like this task");
					String placement = scanin.nextLine();
					int newPlacement = Integer.parseInt(placement);
					task.setOverride(true);
					controller.overrideTask(newPlacement);
